...
from lookups.parsing.quoting import unquote
    ...
    if args is not None:
        try:
            ...
            if 'no closing quotation' in str(ve).lower():
                raise LookupError("error parsing argument string, try quoting the entire line.")
            else:
                raise
...
def split_args(args):
    ...
    if print_depth or block_depth or comment_depth or inside_quotes:
        raise LookupError("failed at splitting arguments, either an unbalanced jinja2 block or quotes: {}".format(args))
